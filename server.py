...
def process_image_async(message_id: str, reply_token: str, user_id: str, source_type: str):
    """
    非同步處理圖片分析（避免 webhook timeout）

    LINE webhook 要求 1 秒內回應 200，
    所以分析放到背景執行緒，用 reply_token 回覆。
    """
    try:
        logger.info(f"開始處理圖片: message_id={message_id}")
        ...
        logger.info(f"回覆完成: user={user_id}")
